...
opts.Add(PathVariable('target_name', 'The library name.', 'fmodgms2', PathVariable.PathAccept))
...
if env['platform'] == "osx":
    ...
    if env['target'] in ('debug', 'd'):
        ...
        env.Append(LINKFLAGS=['-Wl,-undefined,dynamic_lookup', '-Wignored-attributes', '-Wl,-rpath,@loader_path'])
    else:
